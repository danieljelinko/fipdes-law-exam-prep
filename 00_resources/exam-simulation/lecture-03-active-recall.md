# TFCA1402 Food Regulatory Affairs — Active Recall Session
# Lecture 3: Food Information to Consumers — FIC Regulation (EU) 1169/2011

> **How to use:** Copy the entire content of this file (from the line below this box
> to the very end) and paste it as your **first message** into a fresh
> ChatGPT or Claude chat. The AI will ask you exam questions one at a time
> and grade your recall against the lecture material.

---

You are an exam-preparation study partner for TFCA1402 Food Regulatory Affairs
(TU Dublin, MSc Culinary Innovation and Food Product Development, lecturer Sheona
Foley). We are practising **active recall**: you will ask exam questions one at a
time, and I will write what I remember from memory — without looking at any notes.
After each answer, compare my recall against the lecture content I have provided
below and produce a structured assessment.

## Your instructions

1. Read the LECTURE CONTENT and QUESTIONS sections at the bottom of this message.
2. Ask me the **first** question from the QUESTIONS section. State it clearly,
   word-for-word as written. Do **not** reveal the answer outline or any hints.
3. Wait for me to type my free-recall answer before doing anything else.
4. When I have submitted my answer, produce the following three things:

   **a. Comparison table**

   A markdown table with columns: Concept | Recall | Notes.
   - List every key concept, regulation, article number, case example, and
     definition that the source material expects for this question.
   - Mark each row: ✓ (correctly covered), 〜 (partially covered), or
     ✗ (missed entirely).
   - In Notes, say what was good or what was missing — be specific.

   **b. Focused study list**

   A short bullet list of exactly what I need to go back and read, citing
   slide numbers or article numbers where possible.

   **c. Legislation-reference check**

   List every regulation and article number I should have cited in my answer
   but did not. Format citations exactly as the slides do, e.g.:
   "Reg (EC) No 178/2002, Article 7 — Precautionary Principle"
   These citations earn extra marks on the real exam — flag missing ones clearly.

5. After producing the three items above, ask: "Ready for the next question, or
   would you like to revisit this one?"
6. After all questions in the QUESTIONS section have been answered, produce a
   final **session summary table**:

   | Topic area | Performance | Priority to review |
   |---|---|---|
   | e.g. Risk Analysis | Strong / OK / Needs work | Yes / No |

## Rules

- **NEVER reveal the answer, outline, or any hints before I attempt recall.**
  If I ask for the answer, remind me to try from memory first.
- Compare my answers strictly against the LECTURE CONTENT provided below.
  Do not add facts from your training data that are not in the slides.
- Always use the exact regulation citation format from the slides, e.g.
  "Reg (EC) No 852/2004, Article 5" — not shortened forms.
- Be encouraging but precise. The real exam marking deducts for
  "KNOWLEDGE AND REFERENCES MISSING" — make those gaps visible.
- If I write "skip" in response to a question, move to the next question
  without giving the answer, but flag the skipped question in the final
  summary.

---

## LECTURE CONTENT

<<<BEGIN LECTURE CONTENT>>>

# Lecture 3 — Week 4 (18 February 2026): Food Information to Consumers (FIC) Regulation 1169/2011

**Topics:** FIC Reg 1169/2011 · FLIS · date of minimum durability · mandatory information · allergens (14 major allergens) · labelling rules

**Slides:** 125–227 (103 slides)

## Slide 125

![Slide 125](food_law_slides/slide_125.png)

### Food Law & Regulatory Environment — Workshop Week 4

| | |
|---|---|
| **Module code** | TFCA1402 |
| **Workshop** | Week 4 |
| **Date** | 18th February 2026 |

## Slide 126

![Slide 126](food_law_slides/slide_126.png)

### Today's Lecture — Food Information to Consumers (FIC)

Today's topics:

1. **Details of Assignment**
2. **Food Information to Consumers (FIC)**
3. **General Principles**
4. **Mandatory Principles**
5. **Derogations**
6. **Voluntary Terms**
7. **Nutritional Labelling**

## Slide 127

![Slide 127](food_law_slides/slide_127.png)

### Link to FSAI Online E-Modules

**Food Safety Authority of Ireland's Learning Portal** — provides training courses, resources, and materials that can be used in conjunction with this module. Create an account to access a range of courses. Once logged in, click on the tab entitled **"Legal Requirements for Food Businesses Courses"**, where you can find the most relevant e-modules:

1. Food Information for Prepacked Foods
2. Nutrition Information under FIC (Reg 1169/2011)
3. Food Additives — The Basics
4. Food Additives — Labelling
5. Food Flavourings — Interviews with the Experts
6. Introduction to Food Contact Materials
7. Regulation 2073/2005 Module 1: Identifying Relevant Microbiological Criteria
8. Regulation 2073/2005 Module 2: Sampling and Testing

## Slide 128

![Slide 128](food_law_slides/slide_128.png)

### Fairtrade

- Living fairly is a mindset — *it doesn't have to be perfect*. Try, in the month of **Fairbruary**, to buy Fairtrade.
- Get involved by having a little fun whilst supporting our mission to support **Fairtrade farmers**.
- Find Hidden Products or cook a Fairbruary Recipe.

Learn about the new campaign — get involved by playing games and winning prizes.

*Check it out:* [HERE](https://www.fairtrade.ie/).

## Slide 129

![Slide 129](food_law_slides/slide_129.png)

### Assignment (40 %) — *Submission Date: 27 March 2026*

> **Review your food label from a regulatory compliance perspective.**

1. **Introduce your product** — include an image of how it looks.
2. **Consider mandatory information** included and excluded — indicate rationale.
3. **Consider voluntary claims** included and excluded — indicate rationale.
4. **Commentary**.

## Slide 130

![Slide 130](food_law_slides/slide_130.png)

### Food Labelling Information System (FLIS)

> **NEW:** The European Commission has created a **new food labelling information system** which provides a *"user-friendly"* IT solution for food businesses.

Here is the link to the **[FLIS](https://ec.europa.eu/food/safety/labelling_nutrition/food_labelling_information_system_en)**.

## Slide 131

![Slide 131](food_law_slides/slide_131.png)

### Nutritics Software

**Nutritics** software is available to all students. An email with login details has been sent — *have you received it and signed on yet?*

## Slide 132

![Slide 132](food_law_slides/slide_132.png)

### Assignment (40 %)

- Copy of brief with more details on **Brightspace**.
- **FAQs document** on Brightspace — will be updated.
- First, let's look at the legislation.

## Slide 133

![Slide 133](food_law_slides/slide_133.png)

### Legislation around Labelling

1. Labelling laws first introduced **1979** — many documents.
2. New legislation with **Regulation 1169/2011** — now **one document**:
    a. Most rules remain the same.
    b. Some new rules (effective **2014** and **2016**).

## Slide 134

![Slide 134](food_law_slides/slide_134.png)

### Full Title of Legislation

- **REGULATION (EU) No 1169/2011** of the European Parliament and of the Council of **25 October 2011** on the provision of food information to consumers, amending Regulations (EC) No 1924/2006 and (EC) No 1925/2006 of the European Parliament and of the Council, and **repealing** Commission Directive 87/250/EEC, Council Directive 90/496/EEC, Commission Directive 1999/10/EC, Directive 2000/13/EC of the European Parliament and of the Council, Commission Directives 2002/67/EC and 2008/5/EC and Commission Regulation (EC) No 608/2004.
- *EU link:* <http://eur-lex.europa.eu/content/welcome/about.html>
- **National legislation:** S.I. No. 556 of 2014 — *European Union (Provision of Food Information to Consumers) Regulations 2014*.
- *Irish link:* <http://www.irishstatutebook.ie/eli/2015/si/556/made/en/pdf>

## Slide 135

![Slide 135](food_law_slides/slide_135.png)

### Food Information Regulation (EU) No 1169/2011 — Repeals & Amends

**Repeals:**

1. Directive 2000/13/EC — General Labelling
2. Directive 90/496/EEC — Nutrition Labelling
3. Directive 87/250/EEC — Indication of Alcoholic Strength
4. Directive 1999/10/EC — Certain Derogations for QUID
5. Directive 2002/67/EC — Quinine and Caffeine Declarations
6. Directive 2008/5/EC — Additional Compulsory Indications
7. Regulation (EC) No 608/2004 — added phytosterols, phytosterol esters, phytostanols and/or phytostanol esters

**Amends:**

1. Regulation (EC) No 1924/2006 — Nutrition / Health Claims
2. Regulation (EC) No 1925/2006 — Food Fortification

## Slide 136

![Slide 136](food_law_slides/slide_136.png)

### What is "Food Information"?

**Article 2.2(a):**

> *"… information concerning a food and made available to the final consumer by means of a label, other accompanying material, or any other means including **modern technology tools or verbal communication**."*

*(Slide image: magnifying glass over a nutrition label — "It's all in the small print".)*

## Slide 137

![Slide 137](food_law_slides/slide_137.png)

### What is the Objective?

> **To inform consumers as to the exact nature and characteristics of the foodstuff** and enable them to make an **informed choice** when purchasing food.

## Slide 138

![Slide 138](food_law_slides/slide_138.png)

### Labelling, presentation and advertising of foodstuffs must be:

1. **Clear**
2. **Unambiguous**
3. **Must not mislead** the consumer to a material degree
4. **Legible and easy to understand**
5. **In English** *(other languages permitted in addition to English)*

## Slide 139

![Slide 139](food_law_slides/slide_139.png)

### Legibility & Font Size

- **Larger labels:** *x-height* ≥ **1.2 mm**
- **Smaller labels:** if the largest surface area < **80 cm²**, *x-height* ≥ **0.9 mm**

*(Slide image: typography diagram showing **x-height** measured between the **mean line** and **baseline** of the word "Appendix".)*

## Slide 140

![Slide 140](food_law_slides/slide_140.png)

### Application of Rules

1. **Prepacked foods** available for sale to consumers and/or catering trade.
2. **Derogations** for:
    i. Food sold **loose**, or
    ii. Food packaged **on the premises** from where it is sold, or
    iii. Packaged **at the request of the consumer**.
3. **Business-to-Business** sales.

## Slide 141

![Slide 141](food_law_slides/slide_141.png)

### Who do the rules apply to?

1. Applies to **FBO at all stages of the food chain**, where their activities concern the provision of food information to consumers.
2. Includes foods delivered by **mass caterers**, and foods intended for supply to mass caterers.
3. Applies to **catering services provided by transport undertakings** (e.g. airplanes, trains etc.) when the departure takes place on the territories of the Member States to which the Treaties apply.

## Slide 142

![Slide 142](food_law_slides/slide_142.png)

### What is *pre-packed Food*?

**Article 2.1(e):**

- **'Prepacked food'** shall mean any single item for presentation as such to the **final consumer and to mass caterers**, consisting of a food and the packaging into which it was put before being offered for sale, whether such packaging encloses the food completely or only partially, but in any event in such a way that the **contents cannot be altered without opening or changing the packaging**.
- **'Prepacked food' does NOT cover** foods packed on the sales premises **at the consumer's request** or prepacked for **direct sale**.

## Slide 143

![Slide 143](food_law_slides/slide_143.png)

### General Requirements for a Label

1. **Inform** the purchaser about the product.
2. The purchaser must **not be misled**.
3. Information on the label must be:
    i. **Accurate, clear and easy** to understand.
    ii. Easily **visible**, clearly **legible** & **indelible** (where appropriate).
    iii. **Not hidden**, obscured, detracted from, or interrupted by any other written or pictorial matter or any other intervening material.

## Slide 144

![Slide 144](food_law_slides/slide_144.png)

### Article 7 — Food information shall not be misleading

> **1.** Food information **shall not be misleading**, particularly:
>
> a. as to the **characteristics** of the food — in particular as to its nature, identity, properties, composition, quantity, durability, country of origin or place of provenance, method of manufacture or production;
>
> b. by attributing to the food **effects or properties which it does not possess**;
>
> c. by suggesting that the food possesses **special characteristics** when in fact all similar foods possess such characteristics — in particular by specifically emphasising the presence or absence of certain ingredients and/or nutrients;
>
> d. by suggesting, by means of the **appearance, the description or pictorial representations**, the presence of a particular food or an ingredient, while in reality a component naturally present or an ingredient normally used in that food has been **substituted** with a different component or a different ingredient.
>
> *— Article 7*

## Slide 145

![Slide 145](food_law_slides/slide_145.png)

### Food information must not be misleading *(examples)*

- As to **characteristics** of the food — in particular, as to its nature, identity, properties, composition, quantity, durability, country of origin or place of provenance, method of manufacture or production.
- By attributing to the food **effects or properties which it does not possess** *(e.g. weight-losing capabilities)*.
- By suggesting that the food possesses **special characteristics** when in fact **all similar food** possess such characteristics — in particular by specifically emphasising the presence or absence of certain ingredients and/or nutrients *(e.g. oranges containing Vitamin C)*.

## Slide 146

![Slide 146](food_law_slides/slide_146.png)

### Food information must not be misleading *(cont.)*

- By suggesting, by means of the **appearance, description or pictorial representations**, the presence of a particular food or an ingredient, while in reality a **component naturally present** or an ingredient normally used in that food has been **substituted** with a different component or a different ingredient.

**All of the above apply to:**

- **Advertising**
- **Presentation of foods** — in particular their shape, appearance or packaging, the packaging materials used, the way in which they are arranged, and the setting in which they are displayed.

## Slide 147

![Slide 147](food_law_slides/slide_147.png)

### Language

In Ireland, food information must be:

- in **English**, **or**
- in **Irish AND English**.
- Information in other languages may be included **in addition to English**.

## Slide 148

![Slide 148](food_law_slides/slide_148.png)

### Question for NPD — *Exercise*

> Pick sample labels from the selection provided. In groups, consider:

- What information is provided? Make a list.
- Is there any information missing?
- What type of nutritional information is provided?
- Is there any advertising on the label?
- Is any of the information misleading?
- Do you think this information enables a consumer to make an informed choice?

## Slide 149

![Slide 149](food_law_slides/slide_149.png)

### NPD Product

Lets start creating your label

## Slide 150

![Slide 150](food_law_slides/slide_150.png)

### 12 Mandatory Requirements

1. Name of the food *
2. List of ingredients
3. Allergens/intolerances
4. Quantity of certain ingredients (QUID)
5. Net quantity *
6. Date of minimum durability or use by date
7. Any special storage conditions and/or conditions of use
8. Name or business name and address of the FBO
9. Country of origin
10. Instructions for use where necessary
11. Alcoholic strength for beverages with more than 1.2% alcohol by volume *
12. Nutrition declaration

*\* These items must appear in the same field of vision*

## Slide 151

![Slide 151](food_law_slides/slide_151.png)

### Name Under Which Product is Sold

A. Legal name
B. Customary name
C. Descriptive name
D. Trade name/brand name

- Specific provisions – Annex VI

## Slide 152

![Slide 152](food_law_slides/slide_152.png)

### A) The Legal Name

- The legal name is the name specified in Community

legislation for a product meeting particular
specifications, e.g. compositional criteria are
detailed in the Vertical Directives for jam, fruit juice,
chocolate etc.

- If a legal name exists for a foodstuff, this name must

be used on the label

## Slide 153

![Slide 153](food_law_slides/slide_153.png)

### B)  The Customary Name

- In the absence of a legal name, a name customary in the Member

State where the product is sold (to the final consumer) may be used

- Customary name is the name accepted by consumers without needing further explanation e.g. spaghetti, muesli, fish fingers

- Description of product may be needed to avoid confusion

## Slide 154

![Slide 154](food_law_slides/slide_154.png)

### C)  The Descriptive Name

- In certain circumstances (in order to

avoid confusion) a description may be
required to indicate the true nature of
the food and to distinguish it from
- ther foods:

## Slide 155

![Slide 155](food_law_slides/slide_155.png)

### D)  Trade Marks or Brand Names the food for the consumer

- Use of a trade mark or brand name is permitted in addition to the name of

- Such names cannot appear instead of

the legal name or customary name, as it
does not provide sufficient information

## Slide 156

![Slide 156](food_law_slides/slide_156.png)

### E) Specific Provisions – Annex VI *(part 1)*

- **Physical condition** — Details of the physical condition of the food or the specific treatment it has undergone, e.g. powdered, freeze-dried, deep-frozen, refrozen, concentrated, smoked — in all cases where omission could mislead
- **Frozen and sold defrosted** — Name of the food shall be accompanied by the designation 'defrosted'
- **Foods treated with ionising radiation** — 'irradiated' or 'treated with ionising radiation'
- **Normally/naturally present component substituted with different component/ingredient** — in addition to ingredients list, a clear indication of component/ingredient used for the partial/whole substitution

## Slide 157

![Slide 157](food_law_slides/slide_157.png)

### E) Specific Provisions – Annex VI *(part 2)*

- **Meat products/meat preparations/fishery products with added proteins/different animal protein** — name of the food shall bear an indication of the presence of those proteins and of their origin
- **Meat products/meat preparations having appearance of a cut/joint/slice/portion/carcase** — name shall include an indication of presence of added water if it makes up >5% of weight of the finished product
- **Fishery products/prepared fishery products having appearance of a cut/joint/slice/portion/filet/whole fishery product** — name shall include an indication of presence of added water if it makes up >5% of weight of the finished product

## Slide 158

![Slide 158](food_law_slides/slide_158.png)

### E) Specific Provisions – Annex VI *(part 3)*

- **Physical condition** — Details of the physical condition of the food or the specific treatment it has undergone, e.g. powdered, freeze-dried, deep-frozen, refrozen, concentrated, smoked — in all cases where omission could mislead
- **'formed meat' / 'formed fish'** — meat products, meat preparations and fishery products that give impression of whole pieces of meat/fish but consist of different pieces combined together by other ingredients or by other means

## Slide 159

![Slide 159](food_law_slides/slide_159.png)

### 2. The List of Ingredients

**What is an ingredient? — Article 2.1(f)**

> 'ingredient' means any substance or product, including flavourings, food additives and food enzymes, and any constituent of a compound ingredient, used in the manufacture or preparation of a food and **still present in the finished product**, even if in an altered form; residues shall not be considered as 'ingredients'

## Slide 160

![Slide 160](food_law_slides/slide_160.png)

### Ingredients Which Do Not Need to Be Listed

- **Constituents** of an ingredient which have been temporarily separated during manufacture, which are later added but not in excess of the original amounts (e.g. egg)
- **Food additives and food enzymes:**
    - serving no technological function in the final food
    - used as processing aids
- **Carriers/substances not food additives** but used in same way as carriers or processing aids
- **Water** — used in manufacture to reconstitution of an ingredient or in case of liquid medium

## Slide 161

![Slide 161](food_law_slides/slide_161.png)

### 2. The List of Ingredients

1. The word **Ingredients** should be at the beginning of the list
2. All ingredients should be listed in **descending order of weight** (at mixing bowl stage)
3. Names used should be designated by their **specific name**

## Slide 162

![Slide 162](food_law_slides/slide_162.png)

1. Fresh fruit and vegetables, including potatoes, which have not
                        been peeled, cut or similarly treated

Article 19:      2.     Carbonated water, the description of which indicates that it has
                        been carbonated;
3. Fermentation vinegars derived exclusively from a single basic

Foods which      4.
                        product, provided that no other ingredient has been added
                        Cheese, butter, fermented milk and cream, to which no
                        ingredient has been added other than lactic products, food
do not require          enzymes and micro-organism cultures essential to manufacture,
    - r in the case of cheese other than fresh cheese and processed cheese the salt needed for its manufacture

a list of        5.
                      i.
                          Foods consisting of a single ingredient, where:
                             The name of the food is identical to the ingredient name; or

ingredients           ii.    The name of the food enables the nature of the ingredient
                             to be clearly identified
                      e.g. pepper

## Slide 163

![Slide 163](food_law_slides/slide_163.png)

### Compound Ingredients

1. An ingredient which is itself made up of several other ingredients e.g. jam in a biscuit, mayo in a sandwich
2. For example jam can be listed as: *jam (fruit, water, sugar, etc.)* **or** *fruit, water, sugar, etc.*
3. Listing the ingredients of a compound ingredient is **not required** when:
    - i. The composition is defined in Community legislation & where it constitutes <2% of the finished product
    - ii. It consists of mixtures of herbs and/or spices that constitute <2% of the finished product
4. It is a foodstuff for which a list of ingredients is not required under Community legislation, e.g. alcohol

## Slide 164

![Slide 164](food_law_slides/slide_164.png)

### Additives *(Regulation (EC) No 1333/2008)*

- Food additives are substances added intentionally to foodstuffs to perform certain technological functions, for example to colour, to sweeten or to preserve.
- Only those additives that are on the list of authorised food additives may be used.

**Labelling of additives:**

1. Category of food additive (see next slide)
2. Its specific name or its designated E number

**Example:** Antioxidant: Ascorbic Acid *or* Antioxidant: E300

## Slide 165

![Slide 165](food_law_slides/slide_165.png)

### Categories of Additives

- Acid
- Emulsifier
- Modified starch
- Acidity regulator
- Emulsifying salt
- Packaging gas
- Anti-caking agent
- Firming agent
- Preservative
- Anti-foaming agent
- Flavour enhancer
- Propellant Antioxidants
- Flour treatment agent
- Raising agent
- Bulking agent
- Foaming agent
- Sequestrant
- Carrier
- Gelling agent
- Stabiliser
- Colour
- Glazing agent
- Sweetener
- Contrast enhancer
- Humectant
- Thickener

## Slide 166

![Slide 166](food_law_slides/slide_166.png)

### Food additives must be named in the list of ingredients by

1. the functional class of the additive, and

Food            2.    the specific name or designated E number
    - E.g. sodium nitrite, a preservative commonly used in cured meat products, must be listed in the list of ingredients as ‘‘Preservative:

Additives and
                  Sodium Nitrite’ or ‘Preservative: E250’.
    - Sulphur Dioxide: The exception is food products containing sulphur dioxide and sulphites (E220-E228). Sulphur dioxide and sulphites must appear on the label under their chemical names

Sulphur           e.g. sodium metabisulphite, where present at levels exceeding
                  10mg/kg or 10mg/l, expressed as SO2.
    - So, the list of ingredients will show the additive category followed

Dioxide           by the additive name e.g. preservative: sodium
                  metabisulphite. Labelling the category and the additive number
                  is not sufficient. Therefore, it is not enough to label as
                  preservative: E220.
    - Preservative: Sodium Nitrite’ or ‘Preservative: E250’.

## Slide 167

![Slide 167](food_law_slides/slide_167.png)

### Additional Labelling Rules Regarding Additives — Regulation 1333/2008

Foods containing one or more of the following food colours (*) must carry the warning label:

- Sunset yellow (E 110)
- Quinoline yellow (E 104)
- Carmoisine (E 122)
- Allura red (E 129)
- Tartrazine (E 102)
- Ponceau 4R (E 124)

**Required additional labelling:** *'name or E number of the colour(s)': may have an adverse effect on activity and attention in children*

> (*) With the exception of foods where the colour(s) has been used for the purposes of health or other marking on meat products or for stamping or decorative colouring on eggshell

## Slide 168

![Slide 168](food_law_slides/slide_168.png)

### Flavourings

**What are Flavourings?**
'Flavourings' are used in or on foodstuffs to impart a taste and/or odour

**Labelling of Flavours:**
Must be indicated in the list of ingredients by the word 'flavourings' or by a more specific name or description, e.g. 'Flavouring: Vanilla'

## Slide 169

![Slide 169](food_law_slides/slide_169.png)

### Allergens

*Reg 1169/2011 — Annex II: List of ingredients (allergens)*

- List hasn't changed but Commission can amend it if necessary
- Now required to be declared in the **ingredients list** or a separate 'contains' statement if no ingredient list
- Clear reference to the name of the substance or product as listed in Annex II
- Prepacked products — must be **emphasised** through a typeset which clearly distinguishes it from rest of ingredients e.g. means of font, **style**, background colour
- Indication extended to non prepacked

## Slide 170

![Slide 170](food_law_slides/slide_170.png)

### *(Image-only slide — no extractable text.)*

**14 major allergens** — visual grid with product photos for each:

Crustaceans · Eggs · Fish · Peanuts · Soya · Milk · Tree Nuts · Celery · Mustard · Sesame · Sulphites · Lupin · Molluscs · Gluten.

## Slide 171

![Slide 171](food_law_slides/slide_171.png)

### Labelling for Allergens

Visual comparison of old vs. new allergen labelling format. Under Reg 1169/2011, allergens must be **emphasised** (bold, italic, or different background colour) within the ingredients list — not just listed as plain text.

**Old format:** allergens listed without emphasis (e.g. "...Wheatflour, Salt, Cream...").

**New format:** allergens bolded within the full ingredients list (e.g. "...**Wheat**flour, Salt, **Cream**, ...**Celery** Seed...").

## Slide 172

![Slide 172](food_law_slides/slide_172.png)

### Details of Allergens *(Annex II, Reg 1169/2011)*

1. **Cereals containing gluten**, namely: wheat (such as spelt and khorasan wheat), rye, barley, oats or their hybridised strains, and products thereof, *except:*
    - a) wheat based glucose syrups including dextrose
    - b) wheat based maltodextrins
    - c) glucose syrups based on barley
    - d) cereals used for making alcoholic distillates including ethyl alcohol of agricultural origin
2. **Crustaceans** and products thereof
3. **Eggs** and products thereof
4. **Fish** and products thereof, *except:*
    - a) fish gelatine used as carrier for vitamin or carotenoid preparations
    - b) fish gelatine or Isinglass used as fining agent in beer and wine

## Slide 173

![Slide 173](food_law_slides/slide_173.png)

### Details of Allergens b)

5. Peanuts and products thereof
6. Soybeans and products thereof, except:
    - a) fully refined soybean oil and fat (1)
    - b) natural mixed tocopherols (E306), natural D-alpha tocopherol, natural D-

                            alpha tocopherol acetate, and natural D-alpha tocopherol succinate from
                            soybean sources;
    - c) vegetable oils derived phytosterols and phytosterol esters from soybean sources

    - d) plant stanol ester produced from vegetable oil sterols from soybean sources
7. Milk and products thereof (including lactose), except:
    - a) whey used for making alcoholic distillates including ethyl alcohol of

cont.        8.
                            agricultural origin;
                            lactitol;
                       Nuts, namely: almonds (Amygdalus communis L.), hazelnuts (Corylus avellana),
                       walnuts (Juglans regia), cashews (Anacardium occidentale), pecan nuts (Carya
                       illinoinensis (Wangenh.) K. Koch), Brazil nuts (Bertholletia excelsa), pistachio
                       nuts (Pistacia vera), macadamia or Queensland nuts (Macadamia ternifolia), and
                       products thereof, except for nuts used for making alcoholic distillates including
                       ethyl alcohol of agricultural origin;

## Slide 174

![Slide 174](food_law_slides/slide_174.png)

### Details of Allergens *(cont.)*

9. **Celery** and products thereof
10. **Mustard** and products thereof
11. **Sesame seeds** and products thereof
12. **Sulphur dioxide** and sulphites at concentrations of more than 10 mg/kg or 10 mg/litre in terms of the total SO₂, to be calculated for products as proposed ready for consumption or as reconstituted according to the instructions of the manufacturers
13. **Lupin** and products thereof
14. **Molluscs** and products thereof

## Slide 175

![Slide 175](food_law_slides/slide_175.png)

### For cereals cereal name that must be highlighted in the

containing gluten,
is it 'gluten' or the
list of ingredients?

The cereal itself is considered the allergen and
so must be highlighted e.g. wheat, oats, rye etc.
Gluten may be placed in brackets after the name
- f the cereal.

## Slide 176

![Slide 176](food_law_slides/slide_176.png)

### Nuts

**When declaring nuts in the list of ingredients, should the word 'nut' or the name of the nut be highlighted?**

1. The **actual nut** is considered the allergen so this should be highlighted — **not** just the word 'nut' — e.g. hazelnut, almond etc.
2. Pine nuts and coconuts are **not** considered as being part of the category 'nuts and products thereof' and so will not have to be listed as an allergen. However, when used in a food product, they must still be labelled in the list of ingredients.

## Slide 177

![Slide 177](food_law_slides/slide_177.png)

### Pre-packaged foods and allergens       • Contains Box

1. declare in the ingredients list

How are         2.    emphasise through a TYPESET which clearly
                      distinguishes if from the rest of the ingredients for e.g.
                      means of font, style, background colour

    - The use of a ‘contains’ box or statement declaring, for

identified on     example 'contains nuts', is no longer allowed. Allergenic
                  ingredients must now be declared and highlighted in the
                  list of ingredients only.

labels?         • Non Pre-packaged foods
    - Since 13 December 2014, allergens must also be declared
    - n non pre-packaged food e.g. restaurants, delis, canteens, takeaways, cafes, retail outlets etc.,

## Slide 178

![Slide 178](food_law_slides/slide_178.png)

### Quantitative Ingredient Declaration (QUID)

**What is QUID?**
In certain circumstances, it is necessary to state on the label the quantity of an ingredient which is used in the manufacture or preparation of a foodstuff.

- % ingredient shown next to the name of the food, or in the list of ingredients
- QUID is designed to help consumers compare the composition of similar products on ingredients likely to influence their choice

## Slide 179

![Slide 179](food_law_slides/slide_179.png)

### QUID is required when

- The ingredient is included in the name of the food, e.g. cheese &

- nion quiche (cheese & onion must be quantified)

EXAMPLE:  Ingredients:  Whole milk, Pasteurised egg, Wheat flour,
Cheese (15%), Onion (12%), Vegetable oil, Hydrogenated vegetable
- il, Modified maize starch, Salt, Dextrose

- The ingredient is associated with the name of the food such as

‘summer pudding’ (the fruit content must be quantified)

- The ingredient is emphasised in words, pictures or graphics such as

‘with cheese’ (the cheese content must be quantified).

NOTE: a serving suggestion does not need a QUID

## Slide 180

![Slide 180](food_law_slides/slide_180.png)

### QUID Is Not Required When:

1. A **solid foodstuff is presented in a liquid medium** (e.g. kidney beans in salt water) — net weight and drained net weight must be declared; quantity of ingredient can be calculated from weight indications already given
2. **Existing legislation** already requires the quantity to be indicated on the label (e.g. Council Directive 2001/112/EC on Fruit Juices requires: 'fruit content: X% minimum')
3. A **small quantity** (2% or less at mixing bowl stage) of an ingredient or category of ingredient is used for flavouring (e.g. garlic in garlic bread)
4. An ingredient is **in the name of a food** but is in such a small quantity as not to affect consumers' purchasing decisions (e.g. 'malt' in malt whiskey)
5. **No ingredient significantly predominates** by weight, such as in a mixture of fruit or vegetables

## Slide 181

![Slide 181](food_law_slides/slide_181.png)

1. The quantity of an ingredient is calculated on the
                   basis of the recipe at the moment the ingredients
                   are added i.e. at the mixing bowl stage.

2. Where a food has lost moisture following treatment

Calculating        such as cooking, the QUID declaration should
                   correspond to the quantity of the ingredient in the
                   finished product.

QUID          3.   The average quantity of the ingredient or category
    - f ingredients must be stated (should generally be expressed as a percentage that has been rounded to the nearest whole number or to the nearest 0.5 decimal place in those cases where it is below 5%).

## Slide 182

![Slide 182](food_law_slides/slide_182.png)

### Example of QUID — **Butter** Cookie

| Ingredient | Weight (g) |
|---|---|
| Flour | 100 |
| Sugar | 35 |
| **Butter** | **50** |
| Eggs | 10 |
| **Total Weight (at mixing bowl)** | **195** |
| Total Weight (after baking) | 169 |

**Formula for QUID:** 50 ÷ 169 × 100 = **30%**

*(Butter is declared as 30% because the product loses moisture during baking — the denominator is the finished product weight, not the mixing bowl weight.)*

## Slide 183

![Slide 183](food_law_slides/slide_183.png)

### Chilli con Carne Recipe — QUID Exercise

**Ingredients at mixing bowl stage:**

| Ingredient | Weight (g) |
|---|---|
| Minced beef | 300 |
| Kidney beans | 200 |
| Tomatoes | 500 |
| Onion | 50 |
| Celery | 20 |
| Garlic | 20 |
| Spices | 5 |
| Black pepper | 3 |
| Salt | 2 |
| **Total weight** | **1100** |

Assume **10% loss of moisture during cooking**. Calculate the QUID for each ingredient.

## Slide 184

![Slide 184](food_law_slides/slide_184.png)

### Net Quantity

- Weight or volume
- Metric (L/ml, Kg/g)
- Imperial (lb/oz allowed until December 2009 — in addition to metric)

**Exceptions** (net quantity declaration not required):

- Multipacks (that can/cannot be sold individually)
- Foods normally sold by number
- Less than 5 g/ml (except herbs and spices)
- Sugar/confectionery/chocolate products <50 g

**E-mark:** Average quantity (individual units may vary slightly)

## Slide 185

![Slide 185](food_law_slides/slide_185.png)

### Date of Minimum Durability

Regulation 1169/2011 requires that the shelf-life of a foodstuff be indicated by either a date of minimum durability ('best before') or a 'use by' date.

But what is the difference between "Best Before" and "Use By"?

## Slide 186

![Slide 186](food_law_slides/slide_186.png)

### ‘Best before’ date ‘Use by’ date danger to human health durability

Non-perishable foods, long shelf-life
             e.g. dry pasta, biscuits, frozen foods

Date of      From a microbiological point of view are highly perishable
             and likely after a short period to constitute an immediate

minimum      e.g. foods with a short shelf-life that require refrigeration
             fall into this category, such as milk, minced meat, fish,

             and ready-to eat salads.

             Derogations, e.g. chewing gum, vinegar, bakery products
             normally consumed within 24 hours.

## Slide 187

![Slide 187](food_law_slides/slide_187.png)

### Shelf Life: Best Before and Use By

**Shelf-life** is the period of time during which a food maintains its acceptable or desirable characteristics under specified storage and handling conditions — which can relate to the safety or quality of the product and can be microbiological, chemical or physical in nature.

1. **BEST BEFORE** — The date of minimum durability, or 'best before' date, is the date until which a foodstuff retains its specific properties e.g. taste, aroma, appearance, any specific qualities which relate to the product, vitamin content etc. when the product has been stored appropriately and the package unopened. Typically used for canned, dried, ambient, frozen foods etc. Many foods past their 'best before' date may be safe to eat, but their quality may have deteriorated.

2. **USE BY** — In the case of foods which, from a microbiological point of view, are highly perishable and are therefore likely after a short period to constitute an immediate danger to human health, the date of minimum durability must be replaced by the 'use by' date.

## Slide 188

![Slide 188](food_law_slides/slide_188.png)

### Date Declaration

**Use by** — day, month, optionally year

**Best before** — short or long shelf life:

1. < 3 months: Day, Month — e.g. *Best Before: 5th May*
2. 3–18 months: End Month, Year — e.g. *Best before: End August, 2008*
3. > 18 months: End Year — e.g. *Best before: End 2010*

Reference to where the date can be given on the label — see Lid

## Slide 189

![Slide 189](food_law_slides/slide_189.png)

### Date Marking — Exemptions

The following foods are exempt from date marking requirements:

- Fresh fruit and vegetables
- Wines
- Beverages > 10% alcohol
- Bakery products
- Vinegar
- Cooking salt
- Solid sugar
- Chewing gums

## Slide 190

![Slide 190](food_law_slides/slide_190.png)

### Minimum Durability — Some Changes

**Date of minimum durability — changes:**

- Individual portions of ice cream — **no longer exempt**
- Soft drinks, fruit juices/nectars, alcoholic beverages in containers of more than 5 litres — **no longer exempt**

**Frozen meat, frozen meat preparations & frozen unprocessed fishery products** must now show:

- Date of freezing, **or**
- Date of first freezing where product has been frozen more than once (with words "frozen on….")

**'Use by' date expired:** Food considered unsafe — Reg 178/2002

## Slide 191

![Slide 191](food_law_slides/slide_191.png)

### EFSA Guidance

- Guidance on date marking and related food information: **part 1** (date marking) — see page 32 for decision tree
- Guidance on date marking and related food information: **part 2** (food information)

## Slide 192

![Slide 192](food_law_slides/slide_192.png)

### Food Labels and Food Waste

- EUROPEAN COMMISSION Market study on date marking and other

information provided on food labels and food waste prevention Final
Report:

- Reference: European Commission, Directorate-General for Health and Food Safety, Market study on date marking and other information provided on food labels

and food waste prevention – Final report, Publications Office, 2018, https://data.europa.eu/doi/10.2875/808514

## Slide 193

![Slide 193](food_law_slides/slide_193.png)

### Storage Instructions or Conditions of Use

Date 'use by' or 'best before' particulars must be followed by a description of the storage conditions which must be observed.

Enables consumer to **maintain quality** of product, e.g.:

- 'refrigerate after opening'
- 'store in a cool dry place'
- 'keep refrigerated and use within 3 days of opening'

## Slide 194

![Slide 194](food_law_slides/slide_194.png)

### Name or Business Name and Address of the Food Business Operator

- The FBO responsible for the food information shall be the operator under whose name or business name the food is marketed, or, if that operator is not established in the EU, the importer into the EU market
- Contact details must be precise enough to enable a complainant to contact
- Website address on its own is **not acceptable**

## Slide 195

![Slide 195](food_law_slides/slide_195.png)

### Origin

1. Origin required where a failure to indicate it might **mislead the consumer to a material degree** — in particular if information accompanying the food or label as a whole would otherwise imply that the food has a different country of origin or place of provenance (e.g. consumers misled through pictorial representations such as a map or country national flag)

2. Some specific food products have mandatory origin labelling:
    - A limited number of foods must provide country of origin information, e.g. fresh fruit and vegetables, honey, olive oil and fish
    - Meat such as beef and beef products must indicate where the animal was born, raised and slaughtered. From 1 April 2015, meat from pigs, sheep, goats and poultry must indicate the EU Member State or the country outside the EU where the animal was reared and slaughtered (Regulation (EU) No 1337/2013)

3. Quality scheme regulation also applies to origin labelling for some products e.g. beef

## Slide 196

![Slide 196](food_law_slides/slide_196.png)

### Instructions for use

How to use the product so the consumer can make
appropriate use of the food

e.g.  reheat, boil, mix, not suitable for frying.

## Slide 197

![Slide 197](food_law_slides/slide_197.png)

### Alcoholic beverages strength

Alcoholic beverages > 1.2% of alcohol
must indicate the actual alcoholic

Directive 87/250/EEC.

## Slide 198

![Slide 198](food_law_slides/slide_198.png)

### Nutritional Information

## Slide 199

![Slide 199](food_law_slides/slide_199.png)

### You will need to generate nutritional information for you — NPD product.

## Slide 200

![Slide 200](food_law_slides/slide_200.png)

### Nutritional Labelling

## Slide 201

![Slide 201](food_law_slides/slide_201.png)

### Mandatory Nutrition Labelling — Per 100g/mlEnergy                        kJ/kcal

Fat                           g
    - f which saturates         g Carbohydrate                  g
    - f which sugars            g Protein                       g Salt                          g

- Information must appear in this order.
- Where appropriate, a statement indicating that the salt content is exclusively due to the presence of naturally occurring sodium may appear in close proximity to the nutrition declaration. Salt is calculated by mutiplying the sodium content by 2.5.
- Contains negligible amount of … if applicable (see EU guidance on agreed “negligible amounts”

## Slide 202

![Slide 202](food_law_slides/slide_202.png)

### Supplementary Nutrition Information

Mandatory nutrients (green in slide): Energy, Fat, of which saturates, Carbohydrate, of which sugars, Protein, Salt.

**Supplementary nutrients** (optional additions, declared per 100g/ml):

| Nutrient | Notes |
|---|---|
| monounsaturates | Sub-category of Fat |
| polyunsaturates | Sub-category of Fat |
| polyols | Sub-category of Carbohydrate |
| starch | Sub-category of Carbohydrate |
| Fibre | Standalone |
| Vitamins and Minerals | Units specified in Annex XIII; declared as % RI |

> **Note:** This is a **closed list** — other nutrients cannot be added. If a healthy/nutrient claim is authorised and applies to a 'non-listed' nutrient/substance, the amount must be declared near the nutrient list e.g. Omega 3.

## Slide 203

![Slide 203](food_law_slides/slide_203.png)

### Exemptions from Nutrition Declaration

1. Unprocessed products that comprise a single ingredient or category of ingredients e.g. eggs, coffee and sugar
2. Processed products which the only processing they have been subjected to is maturing and that comprise a single ingredient or category of ingredients
3. Waters intended for human consumption, including those where the only added ingredients are carbon dioxide and/or flavourings
4. A herb, a spice or mixtures thereof
5. Salt and salt substitutes
6. Table top sweeteners
7. Products covered by Directive 1999/4/EC of the European Parliament and of the Council of 22 February 1999 relating to coffee extracts and chicory extracts, whole or milled coffee beans and whole or milled decaffeinated coffee beans

## Slide 204

![Slide 204](food_law_slides/slide_204.png)

### Exemptions from Nutrition Declaration *(cont.)*

8. Herbal and fruit infusions, tea, decaffeinated tea, instant or soluble tea or tea extract, which do not contain other added ingredients than flavourings which do not modify the nutritional value of the tea
9. Fermented vinegars and substitutes for vinegar, including those where the only added ingredients are flavourings
10. Flavourings
11. Food additives
12. Processing aids
13. Food enzymes
14. Gelatine
15. Jam setting compounds
16. Yeast
17. Chewing-gums
18. Food in packaging or containers the largest surface of which has an area of less than 25 cm²
19. Food, including handcrafted food, directly supplied by the manufacturer of small quantities of products to the final consumer or to local retail establishments directly supplying the final consumer
20. Beverages containing more than 1.2% by volume of alcohol

## Slide 205

![Slide 205](food_law_slides/slide_205.png)

### information that may be

Nutrition     Where the labelling of a prepacked food
              provides the mandatory nutrition declaration,
              the following information may be repeated:
    - (a)the energy value; or
    - (b) the energy value together with the repeated        amounts of fat, saturates, sugars, and salt.

## Slide 206

![Slide 206](food_law_slides/slide_206.png)

### Inception Impact Assessment labelling indications to certain products reduce food waste

EU’s proposal for a revision of Regulation (EU) No 1169/2011 on the
provision of food information to consumers.
- a proposal for introducing a harmonised mandatory front-of-pack nutrition

- the setting of ‘nutrient profiles’ restricting promotion (via nutrition and

health claims on food labels) of foods which are high in fat, sugar and salt

- consider proposing the extension of mandatory origin or provenance

- revise the current EU rules on date marking (‘use-by’ and ‘best before’) to

Read more here:  https://ec.europa.eu/info/law/better-regulation/have-
your-say/initiatives/12749-Revision-of-food-information-to-consumers-

## Slide 207

![Slide 207](food_law_slides/slide_207.png)

### FSAI Announces Consultation on Food — Labelling

- Information available here:

https://www.fsai.ie/uploadedFiles/Legislation/Consultations/consulta
tion-20210211.pdf

- Online questionnaire available here:

https://www.surveymonkey.com/r/VLXHL7J

Deadline has passed:  25th March 2021

## Slide 208

![Slide 208](food_law_slides/slide_208.png)

### EU Consultation on Labelling say/initiatives/12749-Food-labelling- consultation_en

https://ec.europa.eu/info/law/better-

                                          https://ec.europa.eu/info/law/better-                                                    regulation/have-your-

     Upcoming revisions to address                 regulation/have-your-

  concerns for front-of-pack nutrition    say/initiatives/12749-Food-labelling-    revision-of-rules-on-information-

   labelling, origin labelling and date      revision-of-rules-on-information-                     provided-to-consumers/public-

                 marking.                     provided-to-consumers/public-                                                                                consultation_en

## Slide 209

![Slide 209](food_law_slides/slide_209.png)

### Labelling of Meat

- See guidance note 17
- See links to FSAI website:

https://www.fsai.ie/legislation/food_legislation/fresh_meat/general_
hygiene_provisions.html

More on meat coming up.

## Slide 210

![Slide 210](food_law_slides/slide_210.png)

### Labelling of Meat

- See guidance note 17
- See links to FSAI website:

https://www.fsai.ie/legislation/food_legislation/fresh_meat/general_
hygiene_provisions.html

More on meat coming up.

## Slide 211

![Slide 211](food_law_slides/slide_211.png)

### Salt =Sodium Chloride 1169/2011) you-eating prevention/nutrition/salt

- Salt = Sodium x 2.5 (See Annex 1, Reg

- Salt, not sodium is included in the nutrition

declaration  (Art. 30, Reg. 1169/2011)

- How much salt are you eating?

https://www.safefood.eu/How-much-salt-are-

- EFSA have published draft reference values https://www.efsa.europa.eu/en/interactive-pages/drvs

- EU Science Hub:

https://ec.europa.eu/jrc/en/health-knowledge-gateway/promotion-

## Slide 212

![Slide 212](food_law_slides/slide_212.png)

### DRVs for Sodium and Chloride 2019

The EFSA Panel concludes that a Sodium intake of 2 grams per day is
the adequate intake for the general adult population, while for Chloride
the adequate intake for adults is considered to be 3.1 grams per day.
The given levels of both nutrients corelate since they are obtained from
Salt (Sodium Chloride) as the main dietary source.
Dietary Reference Value for Sodium
https://efsa.onlinelibrary.wiley.com/doi/epdf/10.2903/j.efsa.2019.5778

Dietary Reference Value for Chloride
https://efsa.onlinelibrary.wiley.com/doi/epdf/10.2903/j.efsa.2019.5779

## Slide 213

![Slide 213](food_law_slides/slide_213.png)

### Front of Pack Nutrition Information That May Be Repeated

Where the labelling of a prepacked food provides the mandatory nutrition declaration, the following information may be repeated:

- (a) the energy value; or
- (b) the energy value together with the amounts of fat, saturates, sugars, and salt

## Slide 214

![Slide 214](food_law_slides/slide_214.png)

### Traffic Light Systems — Discussion

*What about the traffic light systems in use by some manufacturers? What do you think?*

See articles on Brightspace.

## Slide 215

![Slide 215](food_law_slides/slide_215.png)

### Allergen Labelling for loose foods

“In Ireland, national legislation stipulates that food allergen
information for non-prepacked food must, as a minimum, be
provided in written format, but can also be provided verbally
at any time. In certain circumstances, written food allergen
information may need to be supplemented by verbal
information, e.g. to assist people with vision impairment or
- ther difficulties”.

FSAI Link:
https://www.fsai.ie/legislation/food_legislation/food_information_fic/allergens.html

## Slide 216

![Slide 216](food_law_slides/slide_216.png)

### Labelling for Allergens

LOOSE FOODS                        CASE: PRET A MANGER

Link to news article: https://www.bbc.com/news/business-45731201

## Slide 217

![Slide 217](food_law_slides/slide_217.png)

- Vegan – no legal definition
    - Artisan, Natural etc. – guidance only
    - Gluten Free – specific rules Voluntary   • Organic – specific rules Terms       • Free Range – specific rules
    - Organic – specific rules
    - Geographic Origin such as PDO, PGI – specific rules

## Slide 218

![Slide 218](food_law_slides/slide_218.png)

### Vegan & Vegetarian

- No legal definition for the terms 'vegetarian' or 'vegan' at European level
- Claims such as, 'suitable for vegetarians' or 'suitable for vegans' are subject to the general laws regarding misleading information etc.
- These terms: 'vegetarian' and 'vegan' in food labelling are used voluntarily by industry. Where these terms are absent, consumers rely on the list of ingredients
- Regulation 1169/2011 mandated that EU set which foods are suitable for vegans and vegetarians (Article 36(3)(b) Regulation (EU) No 1169/2011)
- Some more resources — see Brightspace

## Slide 219

![Slide 219](food_law_slides/slide_219.png)

### Marketing Terms

Food business operators should ensure that the use of certain marketing terms is consistent with the specific provisions laid by **FSAI Guidance Document 29**:

1. Artisan
2. Farmhouse
3. Traditional
4. Natural

## Slide 220

![Slide 220](food_law_slides/slide_220.png)

### Fairtrade

Fairtrade is an alternative approach to conventional trade and is based on a
partnership between some of the most disadvantaged farmers and workers
in the developing world and the people who buy their products.

WHAT DOES THE FAIRTRADE MARK MEAN?
When a product carries the Fairtrade Mark it means it was produced
according to international Fairtrade standards. These standards ensure
Fairtrade products are socially and economically fair and environmentally
responsible. Key standards include the payment of a minimum price and a
premium.

## Slide 221

![Slide 221](food_law_slides/slide_221.png)

### THE FAIRTRADE MINIMUM PRICE

For most Fairtrade goods there is a Fairtrade minimum price which is
set to cover the cost of sustainable production for that product in that
region. If the market price for that product is higher than our minimum
price, then farmers and workers receive the market price.

## Slide 222

![Slide 222](food_law_slides/slide_222.png)

### Labelling of Gluten Free

1. **"gluten free"** — gluten content not exceeding **20 mg/kg or 20 ppm**
2. **"very low gluten"** — gluten content above 20 up to 100 mg/kg

**References:**
- Commission Implementing Regulation (EU) No 828/2014 of 30 July 2014 on the requirements for the provision of information to consumers on the absence or reduced presence of gluten in food
- Article 36(3)(d) of Reg (EU) 1169/2011

## Slide 223

![Slide 223](food_law_slides/slide_223.png)

### What is Organic Food?

- Organic food is the product of an agricultural or aqua cultural system of farming that places a strong emphasis on environmental protection and animal welfare
- Very specific EU and Irish legislation which sets out the criteria
- Must be certified from one of the inspection bodies designated by DAFM

*Ref: Council Regulation (EC) No 834/2007 of 28 June 2007 on organic production and labelling of organic products and repealing Regulation (EEC) No 2092/91, OJ L 189, 20.7.2007, p. 1–23*

## Slide 224

![Slide 224](food_law_slides/slide_224.png)

### Organic Certification Bodies

- The Organic Certification Bodies (OCBs) provide an inspection and

certification service for all Organic Production Units in Ireland. They
have been designated and are regulated by the Organic Unit of the
Department of Agriculture, Food and the Marine (DAFM), and are
responsible for upholding the Organic standards as defined by the EU.

## Slide 225

![Slide 225](food_law_slides/slide_225.png)

### Free Range

The term ‘free range’ legally defined for two
foods – eggs and poultry meat
http://www.agriculture.gov.ie/media/migration/farmingsectors/poultry/guidelinesforfreerangepou
ltryproducersandslaughterhouses72010.pdf

## Slide 226

![Slide 226](food_law_slides/slide_226.png)

### Questions

?

## Slide 227

![Slide 227](food_law_slides/slide_227.png)

### Next Week Labelling continued

---

<a id="lecture-4"></a>

<<<END LECTURE CONTENT>>>

---

## QUESTIONS

<<<BEGIN QUESTIONS>>>

# Lecture 3 — Practice Questions

*Food Information to Consumers (FIC) — Regulation (EU) No 1169/2011*

Source content: [`lessons/lecture-03-week-4-18-february-2026-food-information-to-consumers-fic-regulation-1169-2011.md`](../../lessons/lecture-03-week-4-18-february-2026-food-information-to-consumers-fic-regulation-1169-2011.md)

**Topic mapping (slide 547):** Topic 6 (Allergen Management) sits here; cross-references to Topic 4 (Nutrition declaration) and Topic 8 (voluntary terms like vegan/vegetarian).

**Lecturer's emphasis:** **Very heavily examinable.** Slide 545 carries multiple debate prompts directly from this lecture ("milk/sausage/steak", greenwashing, misleading information). Expect at least 1–2 questions on every paper.

---

## Q1. Can plant-based products use terms like "milk", "sausage", "steak"?

**Topic area:** Misleading information (Topic 1) + Voluntary terms (Topic 8)
**Difficulty:** debate
**Time budget:** 1 hour

### Question prompt

> Critically discuss whether plant-based products should be permitted to use traditionally meat- or dairy-derived terms such as "milk", "sausage", "steak", "cheese". Debate the issue from the perspective of the meat/dairy lobby and from the perspective of vegetarian/vegan advocates, citing the relevant EU rules on misleading information.

### Answer outline

1. **Opening:** Frame as collision between two EU regulations:
   - **Reg (EU) No 1169/2011 Art 7** — food information shall not mislead.
   - **Reg (EU) No 1308/2013** — common organisation of markets in agricultural products — reserves "milk" and dairy terms to milk of mammalian origin (Annex VII Part III).
2. **Main body — meat/dairy lobby perspective:**
   - Consumer protection against confusion.
   - Plant-based products are not nutritionally equivalent — calling them "milk" suggests parity.
   - 1308/2013 already protects "milk", "cream", "butter", "cheese" — no use of dairy terms for non-mammalian products.
   - EU Court of Justice TofuTown ruling (2017) — confirmed dairy terms reserved.
3. **Main body — vegetarian/vegan perspective:**
   - Consumers understand "soya milk", "oat milk" by long usage.
   - Restricting plant-based terms hampers transition to sustainable diets (Farm to Fork Strategy).
   - 1308/2013 only restricts **dairy** terms; meat terms like "sausage", "burger", "steak" are **not currently reserved** under EU law.
   - Amendment 165 (rejected by EP 2020) would have restricted meat names.
4. **Conclusion:** Tension between consumer-protection rationale and sustainability/innovation. Argue whether labelling clarity (e.g. "plant-based burger") is sufficient, or whether reservation is needed.

### Key references to score extra marks
- Reg (EU) No 1169/2011, Article 7 (fair information practices / misleading)
- Reg (EU) No 1308/2013, Annex VII Part III (dairy terms)
- ECJ Case C-422/16 *TofuTown* (2017)
- Farm to Fork Strategy (2020)

---

## Q2. Misleading information — Article 7 of 1169/2011

**Topic area:** Misleading information (Topic 1, Topic 6 cross-over)
**Difficulty:** core (explanatory)
**Time budget:** 1 hour

### Question prompt

> Outline the principal requirements of Article 7 of Regulation (EU) No 1169/2011 concerning fair information practices. With reference to specific examples, discuss how food information may be considered misleading.

### Answer outline

1. **Opening:** Cite Reg 1169/2011 Art 6 (basic requirement that all food for the final consumer be accompanied by food information) and Art 7 (the fair information practices).
2. **Main body — the four-fold Art 7.1 test:**
   - **(a)** as to characteristics — nature, identity, properties, composition, quantity, durability, country of origin, method of manufacture/production.
   - **(b)** by attributing effects/properties the food does not possess (e.g. "weight loss capabilities").
   - **(c)** by suggesting special characteristics when all similar foods possess them (e.g. "oranges containing Vitamin C" — all oranges do).
   - **(d)** by suggesting (via appearance, description, pictorial) the presence of a food/ingredient when in fact a normally-present component has been substituted (e.g. cheese substituted by analogue).
3. **Other Art 7 paragraphs:**
   - **Art 7.2** — info must be accurate, clear, easy to understand.
   - **Art 7.3** — no attribution of medicinal property (preventing/treating/curing disease).
   - **Art 7.4** — applies also to advertising and presentation.
4. **Examples:** Tesco "Everyday Value sweet & sour chicken" (slide 154); "with cheese" claim requiring QUID; "100% natural"/"farm fresh" — judged case-by-case.
5. **Conclusion:** Art 7 sits within a broader regulatory mesh (Art 8 of 178/2002, Reg 1924/2006 for claims). Enforcement remains uneven across MS.

### Key references to score extra marks
- Reg (EU) No 1169/2011, Article 7.1(a)–(d), 7.2, 7.3, 7.4
- Reg (EC) No 178/2002, Article 8 (general consumer protection)
- Reg (EC) No 1924/2006 (nutrition and health claims)

---

## Q3. The 12 mandatory food information particulars

**Topic area:** FIC labelling — mandatory information (Topic 6)
**Difficulty:** core (explanatory)
**Time budget:** 1 hour

### Question prompt

> List and explain the mandatory food information particulars required under Regulation (EU) No 1169/2011. For an NPD (new product development) product of your choice, demonstrate how each requirement would be satisfied on the label.

### Answer outline

1. **Opening:** Cite Reg 1169/2011 Art 9 — mandatory particulars. List the 12 (slide 150):
   1. Name of the food (*)
   2. List of ingredients
   3. Allergens / intolerances (emphasised)
   4. QUID — Quantitative Ingredient Declaration
   5. Net quantity (*)
   6. Date of minimum durability or use-by date
   7. Special storage conditions / conditions of use
   8. Name/business name and address of FBO
   9. Country of origin
   10. Instructions for use where necessary
   11. Alcoholic strength for beverages > 1.2% ABV (*)
   12. Nutrition declaration
   *(* must appear in the same field of vision)*
2. **Main body — apply to chosen NPD product (e.g. ready-meal chicken curry):**
   - Name (legal or customary), ingredients list in descending order, **allergens bolded**, QUID for chicken (% in name), net quantity in metric, "use by" date if perishable, "keep refrigerated", FBO name + address, country of origin (incl. EU origin labelling for meat — Reg 1337/2013), reheat instructions, no alcohol ABV, mandatory nutrition table per 100 g.
3. **Format requirements:** font size ≥ 1.2 mm x-height (Art 13), legibility, no concealing/obscuring.
4. **Conclusion:** Compliance = checklist exercise; the lecturer's "knowledge and references missing" feedback signals that students must cite each particular by Art 9 and explain field-of-vision rule.

### Key references to score extra marks
- Reg (EU) No 1169/2011, Articles 9 (mandatory), 13 (presentation), 14 (distance selling), 21 (allergens), 30 (nutrition declaration)
- Reg (EU) No 1337/2013 (origin labelling for meat — pigs, sheep, goats, poultry)
- FSAI eModules on labelling

---

## Q4. Allergen labelling under FIC

**Topic area:** Allergen Management (Topic 6)
**Difficulty:** explanatory + applied
**Time budget:** 1 hour

### Question prompt

> Discuss the requirements for allergen labelling under Regulation (EU) No 1169/2011. List the 14 allergens listed in Annex II and explain how they must be presented on (a) pre-packed and (b) non pre-packed foods.

### Answer outline

1. **Opening:** Cite Reg 1169/2011 Art 21 + Annex II.
2. **Main body — the 14 allergens (Annex II):**
   1. Cereals containing gluten (wheat, rye, barley, oats, spelt, khorasan)
   2. Crustaceans
   3. Eggs
   4. Fish
   5. Peanuts
   6. Soybeans
   7. Milk (incl. lactose)
   8. Nuts (almonds, hazelnuts, walnuts, cashews, pecans, brazils, pistachios, macadamia)
   9. Celery
   10. Mustard
   11. Sesame seeds
   12. Sulphur dioxide & sulphites (> 10 mg/kg or 10 mg/L SO₂)
   13. Lupin
   14. Molluscs
3. **Presentation rules:**
   - **Pre-packed:** declared in ingredients list AND **emphasised** (bold, italic, contrasting font/colour/background) — Reg 1169/2011 Art 21.
   - "Contains" boxes **no longer allowed** — allergens highlighted within ingredients list only.
   - **Non pre-packed** (since 13 Dec 2014): allergens must also be declared (restaurants, delis, takeaways).
   - For cereals containing gluten: highlight the **cereal name** (e.g. wheat, oats), not the word "gluten".
   - For nuts: highlight the **actual nut name** (hazelnut), not the word "nut".
4. **Pine nuts and coconuts** are NOT in Annex II nuts category (slide 176).
5. **Conclusion:** Allergen labelling is one of the most frequent enforcement issues; FSAI prioritises this. Reg 2021/382 extended food safety culture obligations to allergen management.

### Key references to score extra marks
- Reg (EU) No 1169/2011, Article 21, Annex II (14 allergens)
- Reg (EU) 2021/382 (food safety culture incl. allergen management)
- FSAI guidance on allergens

---

## Q5. QUID — Quantitative Ingredient Declaration

**Topic area:** FIC — QUID & nutrition declaration (Topic 6)
**Difficulty:** applied calculation + explanatory
**Time budget:** 1 hour

### Question prompt

> Explain the QUID rules under Regulation (EU) No 1169/2011. When is QUID required, when is it exempt, and how is it calculated? Illustrate with a worked example.

### Answer outline

1. **Opening:** Cite Reg 1169/2011 Art 22 + Annex VIII. QUID = the percentage of an ingredient, declared on the label.
2. **Main body — QUID is required when:**
   - The ingredient is **included in the name of the food** (e.g. cheese & onion quiche → cheese AND onion must be quantified).
   - The ingredient is **associated** with the name (e.g. "summer pudding" → fruit content).
   - The ingredient is **emphasised** in words, pictures or graphics (e.g. "with cheese" → cheese %).
3. **QUID is not required when** (5 conditions, slide 180):
   - Solid in liquid medium (kidney beans in salt water) — drained net weight already declared.
   - Existing legislation already mandates it (e.g. fruit nectar fruit content under Directive 2001/112/EC).
   - Small quantity (≤ 2%) used for flavouring (garlic in garlic bread).
   - Ingredient in name but in such small quantity as not to affect purchasing decisions (malt in malt whiskey).
   - No ingredient significantly predominates (mixture of fruit/vegetables).
4. **Calculation (slide 182, Butter Cookie example):**
   - Mixing-bowl weight: Flour 100, Sugar 35, Butter 50, Eggs 10 → Total 195 g.
   - After baking: 169 g.
   - QUID = 50 / 169 × 100 = **30 %** (use **finished product** weight as denominator, not mixing-bowl).
5. **Conclusion:** QUID rules are mechanical but easy to fail — the lecturer's chilli con carne exercise (slide 183) tests exactly this.

### Key references to score extra marks
- Reg (EU) No 1169/2011, Article 22, Annex VIII
- Directive 2001/112/EC on fruit juices
- Worked example: mixing-bowl vs finished-product calculation

---

## Q6. Date marking — best before vs use by

**Topic area:** FIC — date marking (Topic 6)
**Difficulty:** explanatory + applied
**Time budget:** 1 hour

### Question prompt

> Distinguish between "best before" and "use by" date marking under Regulation (EU) No 1169/2011. Discuss the link between date marking and food waste, and outline the changes introduced regarding ice cream, large beverage containers, and frozen meat.

### Answer outline

1. **Opening:** Cite Reg 1169/2011 Art 24 — date of minimum durability ("best before") or "use by" for highly perishable foods.
2. **Main body — distinction:**
   - **Best before** — date until which food retains specific properties (taste, aroma, appearance, vitamin content) when stored correctly. Used for canned, dried, ambient, frozen foods. Safe to eat after, but quality may have deteriorated.
   - **Use by** — used for foods that are highly perishable from a microbiological point of view (likely to constitute immediate danger to human health after a short period). E.g. raw milk, minced meat, fish, ready-to-eat salads.
   - **'Use by' expired = food unsafe** under Art 14 of Reg 178/2002.
3. **Format requirements (Art 24 + Annex X):**
   - Use by: day, month, optionally year.
   - Best before: short shelf life (< 3 months) → day, month; 3–18 months → end month + year; > 18 months → end year.
4. **Exemptions (Annex X):** fresh fruit/veg, wines, beverages > 10% alcohol, bakery products consumed within 24 h, vinegar, cooking salt, solid sugar, chewing gum.
5. **Changes / extensions:**
   - Individual portions of ice cream — **no longer exempt**.
   - Soft drinks, fruit juices, alcoholic beverages > 5 L — **no longer exempt**.
   - Frozen meat, frozen meat preparations, frozen unprocessed fishery products — must show date of freezing or "frozen on…" if frozen more than once.
6. **Food waste link:** Confusion between "best before" and "use by" drives consumer waste. EU Commission Market Study on date marking (2018) — proposed revisions under Farm to Fork.
7. **Conclusion:** Date marking is a regulatory tool serving safety, quality, and waste-prevention objectives — sometimes in tension.

### Key references to score extra marks
- Reg (EU) No 1169/2011, Articles 24, 25, Annex X
- Reg (EC) No 178/2002, Article 14 (use-by + unsafe = automatically unsafe)
- EFSA Guidance on date marking (Part 1 — date marking; Part 2 — food information)
- EU Commission Market Study on date marking and food waste prevention (2018, DOI: 10.2875/808514)

---

## Q7. The objective of food information — informed choice

**Topic area:** FIC — overarching principles (Topic 1, Topic 6)
**Difficulty:** debate
**Time budget:** 1 hour

### Question prompt

> "The fundamental objective of food information is to enable consumers to make informed choices." Critically discuss this statement with reference to Regulation (EU) No 1169/2011. To what extent does the regulation actually achieve this objective?

### Answer outline

1. **Opening:** Cite the overarching aim — Reg 1169/2011 Art 3.1: *"high level of protection of consumers' health and interests by providing a basis for final consumers to make informed choices"*.
2. **Main body — supporting:**
   - Mandatory particulars (Art 9) ensure baseline information.
   - Allergen emphasis (Art 21) supports informed choice for allergic consumers.
   - Nutrition declaration (Art 30) supports informed dietary choices.
   - Article 7 prevents misleading information.
3. **Main body — limits:**
   - **Information overload** — small font, dense panels; the "informed consumer" is an idealisation.
   - **Front-of-pack (FoP)** schemes inconsistent across MS — Nutri-Score (France/Belgium) vs traffic light (UK) vs reference intakes (industry-preferred).
   - **Health-literacy gradient** — same label doesn't serve all consumers equally.
   - **Greenwashing** and **voluntary claims** not fully addressed in FIC.
4. **Future direction:** EU's Inception Impact Assessment (slide 206) — proposed FIC revision: harmonised mandatory front-of-pack nutrition labelling, nutrient profiles for HFSS foods, extension of origin labelling, revision of date marking.
5. **Conclusion:** FIC sets the baseline but is increasingly seen as insufficient to meet contemporary consumer-protection goals (especially nutrition and sustainability).

### Key references to score extra marks
- Reg (EU) No 1169/2011, Articles 3, 7, 9, 21, 22, 24, 30
- EU FIC revision Inception Impact Assessment
- FSAI Survey: Irish attitudes to food labelling (2009)
- Nutri-Score; UK traffic-light system

---

## Q8. Greenwashing and sustainability claims on food labels

**Topic area:** Misleading information + Voluntary/sustainability claims (Topics 6 + 8)
**Difficulty:** debate
**Time budget:** 1 hour

### Question prompt

> Critically discuss how voluntary sustainability claims (e.g. "eco-friendly", "carbon-neutral", "sustainable", "ethically sourced") on food labels should be regulated to prevent greenwashing. Refer to the current rules under FIC and any proposed reforms.

### Answer outline

1. **Opening:** Define greenwashing — unsubstantiated or misleading environmental claims. Currently NOT specifically regulated under FIC; falls under Art 7 misleading-information umbrella.
2. **Main body — current EU position:**
   - Reg (EU) No 1169/2011 Art 7 catches misleading **environmental** claims indirectly.
   - Reg (EU) No 2018/848 — organic labelling tightly controlled (certified, EU logo, specific rules).
   - No equivalent for general sustainability terms.
   - Farm to Fork Strategy (2020) — proposed harmonised framework for sustainable food.
3. **Examples of problem areas:**
   - "Country of Origin" usage outside mandatory categories — sometimes used to imply local sustainability.
   - Voluntary "free-from" claims (free-from meat, plant-based, reduced meat).
   - Carbon labelling — voluntary, methodologies vary.
4. **Proposals (EU level):**
   - EU Green Claims Directive (proposed 2023) — pre-approval of voluntary environmental claims.
   - Empowering Consumers for the Green Transition Directive (2024) — bans generic claims like "environmentally friendly" without substantiation.
5. **Conclusion:** Without specific positive rules, sustainability claims sit in a grey zone — Art 7 prohibits misleading but doesn't define what counts as substantiated. Reform direction = pre-substantiation + standardised methodologies.

### Key references to score extra marks
- Reg (EU) No 1169/2011, Article 7
- Reg (EU) 2018/848 (organic production and labelling)
- Reg (EU) No 1308/2013 (marketing of agricultural products — voluntary terms like PDO, PGI, TSG)
- EU Farm to Fork Strategy (2020); EU Green Claims Directive (proposed)
- Empowering Consumers for the Green Transition Directive (2024)

<<<END QUESTIONS>>>

---

Begin now: ask me the first question from the QUESTIONS section above.
